...
def calculate_class_weights(dataset, num_classes, method='inverse'):
    ...
    if method == 'inverse':
        ...
        class_weights = 1.0 / (class_counts + 1e-8)  # adding small epsilon to avoid division by zero
        class_weights = class_weights / np.sum(class_weights) * num_classes
    elif method == 'inverse_square':
        # Inverse square frequency weighting
        class_weights = 1.0 / (np.square(class_counts) + 1e-8)
        class_weights = class_weights / np.sum(class_weights) * num_classes
    else:
...
def train_model(
    ...
    val_set = BasicDataset(dir_img / 'val', dir_mask / 'val', img_scale)
    ...
    class_weights = calculate_class_weights(train_set, n_classes, method=weight_method)
    ...
    experiment.config.update(
        dict(epochs=epochs, batch_size=batch_size, learning_rate=learning_rate,
             ...
             valid_classes=valid_classes, weight_method=weight_method,
    ...
    # Create a weighted cross-entropy loss that ignores the 0 index
...
def get_args():
    ...
    parser.add_argument('--weight-method', type=str, default='inverse', 
                      choices=['inverse', 'inverse_square'],
