...
def process_single_tif(file_path):
    ...
    with rasterio.open(file_path) as src:
        ...
        # Get unique colors and counts including black pixels
        ...
        # Sort both dictionaries by descending value
...
def analyze_existing_jsons(json_folder):
    ...
    for filename in os.listdir(json_folder):
        if not filename.endswith('.json'):  # Remove the biodiversity_ prefix check
            continue
        ...
        # Convert keys to int to ensure proper numerical order, then join with a dash to make a str.
        present_labels = sorted(image_data["label_counts"].keys(), key=lambda x: int(x))
        combo_key = "-".join(present_labels)
...
def main():
    ...
    for filename in sorted(tif_files):
        ...
        # Generate the unique labels string separated by dashes.
        unique_label_str = "-".join(map(str, present_labels))
        
        # Save the string with unique descending label as the value to "label" key
        image_data = {
